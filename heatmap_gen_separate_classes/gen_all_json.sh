...
for files in ${HEATMAP_TXT_OUTPUT_FOLDER}/prediction-*; do
    if [[ "$files" == *.low_res* ]]; then
        ...
        python gen_json_multipleheat.py ${files} ${HEATMAP_VERSION}-high_res ${SVS_INPUT_PATH} lym 0.5 necrosis 0.5 >> ${LOG_OUTPUT_FOLDER}/log.gen_json_multipleheat.txt 2>&1   # HEATMAP_VERSION: neu_v1...
    fi
done
